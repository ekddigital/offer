...
export async function sendTransactionalEmail(input: unknown) {
  ...
  if (
    !env.ANDOFFER_MAIL_API_KEY ||
    env.ANDOFFER_MAIL_API_KEY === "ek_build_placeholder"
  ) {
    console.error(
      "❌ EKDSend API key is not properly configured (using placeholder or missing)",
    );
    throw new Error(
      "Email service is not configured. Please configure ANDOFFER_MAIL_API_KEY.",
    );
  }
